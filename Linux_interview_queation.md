...
What the user reports and what is observable. Includes how to turn a vague complaint into a precise one, and what the exact error message tells you.
...
The first 90 seconds. A short, fixed set of commands that narrows the problem to a layer before you go deep.
...
Ranked root-cause possibilities — most likely first, so you investigate in probability order instead of randomly.
...
Exact commands, configuration files, log locations and out-of-band consoles that confirm or eliminate each cause.
...
Immediate mitigation to restore service, ordered least disruptive first, with the risks of each stated.
...
The proper engineering remediation that addresses the cause rather than the symptom.
...
Monitoring, automation, standards and process changes so it does not recur.
...
network service, ifcfg



NetworkManager + ifcfg



NetworkManager (ifcfg deprecated)
...
Podman (rootless mature)



Podman + image mode / bootc
...
SHA-1 deprecated, OpenSSL 3.0, no TLS 1.0/1.1
...
BIOS/UEFI POST — hardware self-test, finds the boot device from the boot order.



MBR / GPT — first 512 bytes of the disk (446 bytes boot code + 64 bytes partition table + 2 bytes magic). UEFI instead reads the EFI System Partition (/boot/efi).



GRUB 2 — stage 1 in MBR loads stage 1.5/2 from /boot; reads /boot/grub2/grub.cfg, shows the menu, loads the selected vmlinuz kernel and the initramfs into memory.



Kernel — decompresses, initializes memory/CPU/scheduler, mounts initramfs as a temporary root, loads the drivers needed for the real root (storage, RAID, LVM, multipath), then pivots to the real root filesystem read-only, remounts read-write.



init / systemd (PID 1) — kernel executes /sbin/init → symlink to systemd.



systemd targets — reads default.target (usually multi-user.target or graphical.target), resolves dependencies, starts units in parallel, mounts filesystems from /etc/fstab, activates swap, brings up network, starts services.



Login — getty/display manager presents the login prompt; PAM authenticates.
...
data still accessible via the link
...
SSH(22), HTTP(80), HTTPS(443), SMTP(25), FTP(21)



DNS(53), DHCP(67/68), NTP(123), SNMP(161), TFTP(69), syslog(514), streaming/VoIP
...
Wrong password, or password expired — by far the most common, especially after a policy change.



Account locked by failed-attempt policy (pam_faillock) — very common after a script or saved credential retried with a stale password.



Account or password expiry set by chage, often hit in bulk when a compliance policy is applied.



Access restriction — AllowUsers/AllowGroups/DenyUsers in sshd_config, or /etc/security/access.conf, or the user missing from a required group.



Filesystem full or root mounted read-only — sshd cannot write session files, login fails for everyone.



Home directory missing, wrong ownership, or on a failed NFS mount — often after a migration; login succeeds then drops.



Login shell set to /sbin/nologin or /bin/false — typical for a service account someone now wants interactive access to.



SSH key problems — wrong permissions on ~/.ssh or authorized_keys, wrong key, or a bad SELinux context.



Directory service failure — SSSD/LDAP/AD not reachable, cached credentials expired, or Kerberos failing on clock skew.



Firewall, security group or network ACL change — usually affects everyone at once and coincides with a change window.



sshd down, or misconfigured after an edit — the daemon failed to reload.



SELinux denial — after a restore, a manual mv of home directories, or a relabel that never ran.



Resource exhaustion — MaxStartups/MaxSessions reached, PID limit hit, or the box is out of memory.
...
Password placeholder — x means the hash lives in /etc/shadow



UID — 0 = root; system accounts < 1000; users ≥ 1000
...
GECOS/comment — full name, contact info
...
Login shell — /sbin/nologin disables interactive login
...
Encrypted password — $6$ = SHA-512, $5$ = SHA-256, $1$ = MD5, $y$ = yescrypt. ! or !! = locked, * = no password login, empty = no password at all.
...
Inactive days — grace period after expiry before the account is disabled
...
Volume group or logical volume not active — the most common after a reboot or SAN maintenance. lvs shows no a in the Attr column.



Wrong or stale /etc/fstab entry — a UUID that changed after a mkfs, a typo, or the wrong filesystem type. Very common after a rebuild or restore.



Underlying PV or SAN path missing — LUN unmapped, zoning change, multipath path failure. pvs reports "unknown device".



Filesystem corruption or a dirty log after an unclean shutdown or storage outage.



Device-mapper nodes missing under /dev/mapper even though LVM metadata is fine, usually a udev issue.



No filesystem on the LV at all — someone created the LV but never ran mkfs, or the wrong device was formatted.



Mount point directory does not exist, or is not empty and is shadowing data.



Duplicate XFS UUID — a cloned VM or a snapshot presented alongside the original.



Read-only or failing hardware — the kernel remounted read-only after I/O errors.



Already mounted at another path, so the second mount looks like a failure.



SELinux context wrong on the mount point — mounts, but the application is denied.
...
Application or system logs that are not rotating — the single most common cause. Either logrotate is not configured for that log, or the app holds the file handle so rotation frees nothing.



A deleted file still held open by a running process — someone already "cleaned up" by deleting the log, which freed zero space.



A sudden application problem generating huge logs — a loop writing stack traces, debug logging left enabled after troubleshooting, or an audit rule flooding /var/log/audit.



Core dumps from a repeatedly crashing process, often multi-gigabyte each.



Old kernels and package cache filling /boot or /var — /boot filling is why kernel updates fail.



Temp files never cleaned in /tmp, /var/tmp, or an application's own temp directory.



A backup, export, dump or transfer written to the wrong location — a database dump landing on / instead of the data volume.



Container/image sprawl in /var/lib/docker or /var/lib/containers.



Inode exhaustion from millions of small files — mail queues, session files, cache directories.



Genuine organic growth — the data really did outgrow the volume, and the answer is capacity, not cleanup.



A snapshot filling up (LVM snapshot or VM-level), consuming space as the origin changes.
...
A single runaway application process — an infinite loop, a stuck thread pool, a bad query plan burning CPU.



A scheduled job overlapping production load — backup, antivirus scan, patch job, index rebuild, log compression, or a cron job that now runs longer than its window and collides with the next run.



Legitimate load growth — more users or transactions than the box was sized for.



High %iowait misread as a CPU problem — the CPU is idle and waiting on storage; fixing CPU will do nothing.



High %steal on a virtual machine — the hypervisor is oversubscribed; this is a virtualization-team issue.



High %sys / interrupt load — network flood, driver problem, excessive context switching, or a chatty syscall pattern.



Swap thrashing showing up as CPU load — memory pressure causing constant paging.



A fork bomb or process leak — thousands of processes or threads spawned.
...
Application memory leak — usage grows steadily and never returns after restart.



Misconfigured application memory — JVM heap or database buffer pool sized larger than physical RAM, connection pools too large, too many worker processes.



Too many concurrent processes/sessions — each modest, but multiplied.



Filesystem cache misread as a problem — Linux uses free RAM for cache by design; this is healthy and reclaimable.



Insufficient or disabled swap turning a survivable pressure event into an OOM kill.



Kernel-side growth — slab/dentry cache growth, or a leaking kernel module.



Undersized VM — the workload genuinely needs more RAM.



A backup or data-processing job holding large buffers.
...
Memory exhaustion / swap thrashing — the box is alive but spending all its time paging; often accompanied by OOM kills. This is the most common cause of an apparent hang.



Hung storage — a failed SAN path, an unresponsive NFS server, or a dying local disk. Processes pile up in D state, load average climbs into the hundreds while CPU sits idle.



Full or read-only root filesystem — nothing can write, so almost everything stalls, including login.



A runaway process consuming all CPU or forking uncontrollably — a fork bomb or process leak exhausting the PID/task limit so no new command can start.



Kernel or driver bug — a soft lockup, hung task, or deadlock. dmesg shows "BUG: soft lockup", "task blocked for more than 120 seconds", or a call trace.



Hardware fault — failing DIMM with correctable-error storms, CPU fault, failing disk, or a thermal event. The iLO/iDRAC system event log is the authority here.



File descriptor or resource limit exhaustion — cannot open files or sockets, so services stall.



Hypervisor-level problem — the underlying host is overloaded, its storage is unavailable, or the VM is stunned during a snapshot operation. The guest looks hung but is a victim.



Network-layer problem only — the server is fine and the network path is broken; the console will prove this in seconds.
...
Read and photograph the full panic output, including the call trace and the last few lines before it.



Note whether the panic happens immediately at boot (before any services) or later in the boot. Early means root device or initramfs; later means filesystem, driver or userspace.



Reboot and watch the GRUB menu: is there a previous kernel entry available? That is your fastest recovery.



Check whether the server is physical or virtual, and whether other servers patched in the same batch are also affected — a pattern means a bad kernel or a storage change, not a one-off.



Check the change record for what happened in the last 24 hours.
...
Bad or incomplete kernel update — the new kernel's initramfs was built without the storage driver, or /boot was full so the initramfs is truncated. Very common.



Missing storage driver in initramfs — multipath, HBA, RAID controller or virtio driver not included, so the root device is invisible.



Wrong kernel command line — root=UUID= or rd.lvm.lv= no longer matches reality after a restore, clone, or filesystem recreation.



Root device genuinely unavailable — SAN LUN unmapped, zoning changed, multipath misconfigured, boot disk failed.



Corrupted root filesystem after an unclean shutdown or storage outage.



Bad /etc/fstab — in some cases this panics rather than dropping to emergency mode, particularly if root itself is involved.



Faulty hardware — uncorrectable ECC memory errors, failing CPU, or a failing boot disk. Machine check exceptions point here.



Kernel or driver bug triggered by a specific workload or a newly loaded module.



Incompatible or unsigned third-party module (backup agent, antivirus, monitoring, GPU driver) built against a different kernel.



Full /boot — worth calling out separately because it causes several of the above at once.
...
Read the exact error string — it distinguishes "MBR gone" from "grub.cfg missing" from "/boot corrupted", and each has a different fix.



Check the firmware boot order and whether the boot disk is even detected — in BIOS/UEFI setup or via iLO/iDRAC. A surprising number of "GRUB corrupted" cases are a boot-order change or a disk that dropped offline.



Determine BIOS/legacy or UEFI, because the repair procedure differs significantly. In UEFI setup you will see boot entries like "Red Hat Enterprise Linux"; the presence of an EFI System Partition is the giveaway.



If you get a grub> or grub rescue> prompt, use it to look around immediately — you may be able to boot manually within a minute.



Confirm you have rescue media (matching RHEL major version ISO) and console access before planning the repair.
...
MBR / boot sector overwritten — another OS installation, a cloning tool, a misdirected dd, or a bootloader written to the wrong disk.



/boot contents missing or corrupted — a full /boot during a kernel update, an interrupted update, or filesystem corruption.



grub.cfg missing, empty or invalid — hand-edited, or grub2-mkconfig run with a broken /etc/default/grub.



Disk order or device naming changed — a disk added, removed or re-presented, so GRUB's saved (hd0,msdos1) no longer points where it should.



UEFI boot entry lost — NVRAM cleared, battery replaced, firmware updated or reset, or the EFI System Partition not mounted when a GRUB update ran.



Failing boot disk — bad sectors precisely where the bootloader lives.



A restore or clone where /boot, the fstab UUIDs and the bootloader no longer agree.



Wrong grub2-install target — installed to a partition or a mirror member rather than the boot disk, or run on a UEFI system where it is not appropriate.
...
Bad /etc/fstab entry — a typo, a stale UUID after a mkfs or restore, the wrong filesystem type, or an entry for a device that no longer exists. By far the most common, and usually the result of an edit that was never tested with mount -a.



Volume group or logical volume not activated at boot, so the device never appears.



Missing or unavailable device — a SAN LUN unmapped, an iSCSI target unreachable, a USB or external disk removed, or a NFS mount without _netdev attempted before the network is up.



Filesystem corruption — the mount fails or a required fsck failed, typically after an unclean shutdown or storage outage.



Root filesystem full — systemd cannot write what it needs, so the boot fails in confusing ways.



Root mounted read-only because of I/O errors, which then cascades into service failures.



A failed fsck on an ext filesystem at boot, which deliberately drops you to a shell for manual repair.



A required systemd unit failing that other units depend on (rarer, but possible after a configuration change or a broken package).



SELinux relabel problems or a missing critical directory after a bad restore.



Corrupted or missing initramfs / wrong kernel parameters — if the failure is earlier than this, you get a panic or a dracut shell instead.
...
Reboot; at the GRUB menu press e to edit the default entry.



Find the line starting with linux16 / linux/linuxefi, go to the end, and add:
...
Press Ctrl+X to boot. You land in the initramfs shell with the real root mounted read-only at /sysroot.
...
Storage latency — the most common and the most frequently misdiagnosed. SAN congestion, a failed multipath path, a degraded RAID rebuild, a snapshot consolidation, or a failing disk. Shows as high %iowait and high await with idle CPU.



Memory pressure and swapping — the system is alive but paging constantly, which looks like everything being slow at once.



A competing scheduled job — backup, antivirus scan, log compression, patch job, database index rebuild, or a report that now overlaps the business day. This is why "it is slow every night at 1 a.m." is a solved case before you even log in.



Application-level problem — a bad query plan after statistics changed, a missing index, thread-pool exhaustion, connection-pool starvation, garbage-collection pauses, or a memory leak.



Hypervisor contention — CPU ready time, memory ballooning, or datastore latency caused by other VMs. %steal is your evidence.



CPU saturation — genuine, from load growth or a runaway process.



Network problems — packet loss, retransmissions, a duplex or speed mismatch, MTU issues, a saturated uplink, or DNS timeouts (which make everything feel slow while showing no resource usage at all).



A slow dependency — an NFS server, database, LDAP/AD, or an API the application calls. The server is fine and waiting.



Filesystem nearly full or fragmented, especially XFS above 90%.



Too many concurrent users or sessions for the sizing.



A monitoring or security agent misbehaving — agents are a real and often-overlooked cause.



Hardware degradation — correctable memory errors, a throttling CPU due to thermal or power capping, or a degraded RAID cache battery causing write-through mode. That last one is a classic: a failed cache battery can slow a database by an order of magnitude.
...
run as file group / inherit group
...
Cannot reach the server — timeout, no route to host, connection refused. This is network, firewall or host-down.



Reaches it but cannot log in — permission denied, access denied, immediate disconnect. This is authentication and authorization, covered in detail in Q6.



Logs in but cannot access something — a directory, a file, a command, a mount, or a service. This is permissions, sudo, SELinux or a missing mount.
...
Firewall, security-group or network ACL change — often affects everyone simultaneously and correlates with a change window.



Server down, hung, or in emergency mode after a reboot or patch.



DNS resolving to the wrong or an old IP — very common after a migration or IP change.



Network path problem — routing, VPN not connected, a VLAN or switch change, a failed NIC or bond member.



sshd not running or bound to the wrong address/port, typically after a configuration change or a failed patch.



Full or read-only root filesystem preventing sshd from creating sessions.



Resource exhaustion — MaxStartups reached, out of memory, PID exhaustion, so connections are refused or hang.



TCP wrappers (/etc/hosts.allow, /etc/hosts.deny) on older builds.
...
Filesystem permissions or group membership — the user is not in the required group, or was added but has not re-logged-in so the new group is not in their session token.



ACLs or SELinux context denying access despite correct-looking ls -l permissions.



A mount not present — the data directory is empty because the NFS or LVM mount failed.



sudo rules missing or too narrow.



The application's own authorization, not the OS at all.
...
You or another admin has a shell whose working directory is inside the mount — the most common and the most embarrassing. Includes forgotten screen/tmux sessions.



A running service or application has files open on the filesystem — database, web server, application server, log writer.



A running executable or shared library is loaded from the mount — you cannot unmount the filesystem a running binary came from.



A nested mount or bind mount underneath the mount point — including /data/sub mounted separately, or a bind mount elsewhere referencing it.



A background job you did not think of — backup agent, antivirus scan, monitoring plugin, find from a cron job, log shipper, or an indexing process.



A tail -f, less, or editor left open by someone, often in a detached session.



A swap file located on the filesystem.



The filesystem is NFS-exported from this server, so the kernel holds it.



A loop device backed by a file on the mount, or a device-mapper target on top of it.



A container (Podman/Docker) with a volume or bind mount into it.



A hung NFS mount where processes are stuck in uninterruptible D state and cannot be killed at all.



Deleted-but-open files still referenced by a process.
